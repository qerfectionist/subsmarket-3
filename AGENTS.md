...
изолирован в auth_db), затем получают fresh user в основной сессии через
`db.get(User, user.id)`.
...
### 12. Undo для удаления участника (deferred removal)

**Проблема:** `remove_member` удалял участника сразу — случайный тап владельца
приводил к необратимому действию.

**Решение (backend):**
- `remove_member` (`families/members.py`) теперь переводит member в
  `removal_pending` + `removal_scheduled_at = now + 12h` вместо `removed`.
  Не отменяет платежи и не освобождает место сразу.
- `execute_member_removals` (`jobs/service.py`) довершает удаление через 12h
  (инфраструктура уже была).
- `revoke_member_removal` отменяет — возвращает `active`.
- Audit action: `family_member_removal_scheduled` (вместо `..._removed_by_owner`).

**Решение (frontend):**
- `revokeMemberRemoval` API + `useRevokeMemberRemoval` hook.
- `App.tsx` — после `remove_member` показывается `Snackbar` (8s) с кнопкой
  "Отменить" → `revokeRemovalMutation`. Если не нажать — удаление довершится
  jobs через 12h.
- `undoRemoval` state хранит `{familyId, memberId}` для undo action.

**Затронутые файлы:**
- `families/members.py` — `remove_member` → deferred.
- `tests/test_family_service.py` — 3 теста обновлены на deferred.
- `tests/test_family_api.py` — integration test обновлён.
- `api.ts` — `revokeMemberRemoval`.
- `hooks/useApi.ts` — `useRevokeMemberRemoval`.
- `App.tsx` — `undoRemoval` state + Snackbar с Undo action.

**Конвенция:**
- Деструктивные действия — deferred (с окном для undo), не immediate.
- Snackbar с Undo action — 8s окно, кнопка "Отменить".
...
- `frontend/tests/family-flow.spec.ts` — 6 тестов:
  ...
  6. `undo member removal via snackbar restores the member` — после
     remove_member показывается Snackbar (8s) с кнопкой "Отменить"; тап
     возвращает участника в `active`, `remove-member-button` снова виден.
...
- `App.tsx` — `Snackbar.Button` Undo получил `data-testid="undo-removal-button"`
  для стабильного селектора (telegram-ui рендерит несколько кнопок с текстом
  "Отменить", `getByRole` неоднозначен).
- `App.tsx` — после `revokeRemovalMutation` вызывается `loadOwnerDetails`,
  т.к. `ownerDetails` — manual `useState`, не React Query. Без этого
  список участников не обновлялся после Undo.
- `hooks/useApi.ts` — `useRevokeMemberRemoval` теперь инвалидирует
  `familyView` (раньше только `ownerRequests` и `familyMembers`).

**Затронутые файлы:**
- `frontend/tests/family-flow.spec.ts` — новый файл, 6 тестов + хелперы.
- `frontend/tests/tma-smoke.spec.ts` — существующий, 1 тест.
- `frontend/src/components/families.tsx` — `useEffect` для auto-switch табы.
- `frontend/src/App.tsx` — `data-testid` на Undo button + `loadOwnerDetails`
  после revoke.
- `frontend/src/hooks/useApi.ts` — `familyView` инвалидация в
...
- Manual `useState`-состояния (типа `ownerDetails`) требуют явного
  refresh после mutation — React Query инвалидация их не обновит.
...
- Новые frontend API вызовы — hook в `hooks/useApi.ts`.
