...
## Marketplace Engine: аккаунты

Миграция `20260720_0030` добавляет отдельные таблицы:

- `marketplace_account_services` - справочник ChatGPT, Gemini, Grok, Canva;
- `marketplace_account_listings` - продавец, сервис, название предложения,
  цена, описание, статус и 30-дневный срок;
- `marketplace_account_requests` - покупатель, снимок объявления, состояние
  контакта и результат `sold|not_sold`.

Остаток и количество товара не хранятся. Один покупатель может иметь только
одну активную заявку на объявление, но разные покупатели не блокируют друг
друга. Принятие заявки не закрывает объявление. Логины, пароли и банковские
реквизиты в этих таблицах отсутствуют. На PostgreSQL включен RLS; прямой доступ
ролей Supabase закрывается, если эти роли существуют в конкретной базе.
